...
from keyboards.inline import confirm, full_signup, successful, variants_buttons
from loader import dp, db, bot
from states import FullSignupState, TestYechishStateGroup
...
async def solution_test(msg: types.Message, state: FSMContext):
    ...
    if user[3] is None:
        ...
        await msg.answer("Test yechish uchun email va tug'ilgan kuningizni kiriting:", reply_markup=full_signup)
        await state.set_state(FullSignupState.register)
    else:
        await msg.answer("Test yechishni boshlang!", reply_markup=ReplyKeyboardRemove())
        await msg.answer("Tayyormisiz?", reply_markup=successful)
        await TestYechishStateGroup.success.set()


@dp.callback_query_handler(text="successful_test", state=TestYechishStateGroup.success)
